 ...
 * 原注释逐字随迁（纯移动、零行为变更）。
 *
 * 用户迭代 2026-09-11：**撤下拉与修改**——头部卡的展开箭头/展开区与「编辑」
 * 就地编辑整条链一并撤除（用户原话「去掉主任务和子任务的下拉和修改按钮」）。
 * 卡面只剩：主题 + 状态 pill + 执行人头像 + 指派灰注 + 文件夹行 + extra 槽 +
 * actions 槽（主任务页放开始/暂停钮）。
 *
 ...
 * 组详情页头部「开始」钮与任务详情页头部「编辑」钮。三十三轮 DA46：撤
 * #id 前缀（用户拍板「把 #1 去掉」）。2026-09-11：撤展开钮/展开区与就地
 * 编辑槽（下拉与修改整链撤除），卡面只剩静态信息 + extra/actions。 */
