 ...
 * 用户迭代 2026-09-15（二）→ 2026-09-16 修订：格子 hover 动画初版落「同色柔光」
 * （brightness/saturate + currentColor drop-shadow），用户随即指出「应该是边框
 * 变浅蓝色，而不是变白」——柔光的 brightness() 会把浅色/空档填充乘到近纯白、
 * 在白卡上等于消失；改为**只换描边色、填充与占位不动**（描边宽度两态常驻占位，
 * 符合「视觉微调不改变元素占位」），见 ensureUsageCalendarStyles 覆盖三。
...
function ensureUsageCalendarStyles(): void {
  ...
  try {
    ...
    style.textContent =
      ...
      // 覆盖三：格子 hover 描边亮起（用户迭代 2026-09-16「应该是边框变浅蓝色，
      // 而不是变白」，推翻 2026-09-15 的同色柔光档）：原 brightness() 会把浅色档
      // (#bae6fd) 与空档 (#f1f5f9) 的填充乘到近纯白、在白卡上等于消失，故改为
      // **只换描边色、填充不动**。常驻 `stroke:transparent` + `stroke-width:1` 占位
      // （透明↔蓝只换色，格子尺寸与摆位零变化，几何风险与柔光档同级）；`!important`
      // 用于压过 usageFlatBlock 内联的 `stroke:none`。强调色取**色板外**的两档，
      // 保证与每一档填充都有对比——浅色主题 #0284c7 比色板最亮档 (#0ea5e9) 更深、
      // 暗色主题 #7dd3fc 比最亮档 (#38bdf8) 更浅；若直接取格色/主色，命中对应档
      // 时描边与填充同色、hover 等于没画（本文件色板见 USAGE_CALENDAR_THEME）。
      // 选择器限定带 data-date 的数据格，图例色块（无该属性）不受影响。
      // prefers-reduced-motion 只关过渡、留瞬时反馈。
      '.react-activity-calendar__calendar rect[data-date]{stroke:transparent !important;stroke-width:1;transition:stroke .18s ease;}' +
      '.react-activity-calendar__calendar rect[data-date]:hover{stroke:#0284c7 !important;}' +
      'body[data-ds-dark-theme] .react-activity-calendar__calendar rect[data-date]:hover{stroke:#7dd3fc !important;}' +
 ...
 * docs/28.5.2 几何行）。
 * hover 描边（用户迭代 2026-09-16「边框变浅蓝色」）：由 ensureUsageCalendarStyles
 * 覆盖三经 `!important` 压过这里的 `stroke:none` 后接管（透明↔强调蓝只换描边色），
 * 故此处不再为「同色柔光」注入 `color`——原 currentColor 光晕取色随该档一并撤除。 */
