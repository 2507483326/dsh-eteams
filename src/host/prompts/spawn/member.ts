...
export const MEMBER_RULES = [
  ...
  '留言板（用户 2026-09-14）：**开工前先读队伍留言板**（绝对路径见简报）——看别人做到哪、有什么交接与坑；**做完/交接时在末尾追加一行**「- [时间] 你的名字：做了什么（结论/交接物）」。只记要点，不要长篇复述。',
...
/**
 * 通用成员简报（用户迭代 2026-09-11；2026-09-14 增「队伍留言板」；2026-09-15
 * 扁平化改「工程根 + 任务目录 + 纪要」）：每个任务成员的出生包与每次指派信都
 * 由本模板包住，固定写明五件事——①工程根（代码产出写这里）与任务目录②队伍
 * 留言板（绝对路径 + 读/写纪律）③本任务的纪要（绝对路径 + 写在哪一段）④文档
 * 产出目录⑤领队（无领队=主会话）与三节点实时汇报（开工/遇问题/完成）。抽成
 * 纯文本函数供两路复用，避免字面值双轨（runtime 侧组装好值后传参，本函数不
 * 读盘）。
 */
export function memberBriefing(opts: {
  ...
  taskDir: string;
  /** 队伍留言板绝对路径（boardFileAbs：主任务目录根下的 留言板.md）。 */
  boardFile: string;
  /** 本任务纪要绝对路径（`<任务号>-slug.纪要.md`）。 */
  minutesFile: string;
  /** 文档目录绝对路径（交付说明 / 验收结论这类文档产出）。 */
  docDir: string;
}): string {
  return [
    '## 你的工作目录',
    ...
    '## 你这份任务的纪要',
    `- ${opts.minutesFile}`,
    '- 在你这份文件末尾的 `## 纪要正文` 之下写：做了什么 / 改了哪些文件 / 如何验证 / 遗留。上半段是宿主自动渲染的合同视图，**不要手工编辑、也不要另建纪要文件**。',
    '',
    '## 文档产出',
    `- 交付说明 / 验收结论 / 调研笔记这类文档写进：${opts.docDir}（文件名自定）。`,
    '',
    '## 你的领队',
...
export function memberWelcome(
  ...
  return [
    ...
    MEMBER_TOOL_SHEET,
    '',
    '任务详情用 eteams_task_board 查看；任务目录与纪要的绝对路径见上方简报（一个主任务一个目录）。现在等待第一条指派。',
  ].join('\n');
}