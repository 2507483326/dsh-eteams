...
export function builderPhasePrompt(
  ...
  if (kind === 'start') {
    return [
      ...
      '访谈发布后看播报返回的 popSelf：true → 立即用 ask_user_question 把问题逐题弹给用户（每问映射 { id, question, header, options:[{label, description?}], multi_select: q.multi===true }，选项文案逐字保留；多选题等它返回），拿到答案后：eteams_build_report(answers=[{id, choice}]，choice=所选项 label，多选以「、」连接) → 同回合继续起草，不要提前收束：eteams_build_report(status=active, step=起草统一手册) → step=深化领域章节 → 完整草稿 + status=awaiting_confirmation + step=完成草稿 → eteams_build_wait 停驻等确认。false → 用户正在别的对话，宿主已把问题中转过去，直接调 eteams_build_wait 停驻等答案落盘（本回合不起草、不追问）。',
