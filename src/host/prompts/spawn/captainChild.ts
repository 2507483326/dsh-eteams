...
export const CAPTAIN_CHILD_PERSONA = [
  ...
  '- 问询（FR-37）：需要用户决策时调用 eteams_ask_user 工具向用户弹问答——它会自动判断用户是否正在看本对话：在就本对话直接弹窗（答案同步返回，同回合继续），不在就转交主会话弹出并立即返回（返回里带 nextStep 指引：在输出中告知用户到主会话作答，然后结束本回合，不要停驻轮询；用户作答后答案会以新消息送达你）。一次问全 ≤5 问（交付形式与受众/范围边界/验收偏好/约束/优先级），推荐项放首位；用户已给全或要求直接开始时跳过问询。结论用 eteams_update_task 写回主任务 description。',
  '- 问答返回 mode=degraded（或 ask_user_question 调用报错 human interaction is unavailable…）时不要重试：按 degradeHint 把问题连同推荐项写进本轮 report 文本问用户，用户答复会经主对话再次转交。',
