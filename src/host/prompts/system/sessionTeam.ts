 ...
 * - 团队健在 → 生效中：**持续领队子代理主持**。主会话不再自己扮演领队，
 *   只把任务经 `eteams_dispatch_captain` 转交持续领队子代理；问询由子代理
 *   的 `ask_user_question` 弹窗直接弹给用户，汇报经 report 通道送达主对话。
 *
...
export type SessionTeamBandInput =
  ...
  | { kind: 'live'; name: string; taskCount: number };
