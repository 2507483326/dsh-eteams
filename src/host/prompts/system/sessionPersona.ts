/**
 * 角色接管 band 文本组装（docs/13.8.2）：the panel selects a member for a
 * session and the session agent's system prompt gains a persona band —
 * evaluated per assembly, keyed by the assembling agent. 纯文本函数：会话
 * 人设查表由 runtime 薄壳（runtime/sessionPersona.ts）完成后传参，这里不
 * 触状态（依赖方向 prompts ← runtime，见 prompts/README.md）。
 *
 * @module dsh-eteams/prompts/system/sessionPersona
 */
...
/**
 * The dynamic band text for one session with an active persona（runtime 薄壳
 * 查到人设后委托；空段语义在薄壳里——无会话/无人设直接给 ''，不进本函数）。
 *
 ...
 */
export function sessionPersonaBand(persona: SessionPersona): string {
  const role =
