...
export const ROLE_BUILDER_SECTION = [
  ...
  '- 「后台构建代理纪律」（宿主已内置，此处仅备查）：构建由**一个持续（可续聊）子代理**执行——受理回合（查重 → 发布意图访谈 → 弹窗作答 → 起草到 awaiting_confirmation 待确认）之后，宿主以 followup 续聊同一个子代理（面板提交访谈答案/「继续构建」/重启都只是唤醒它，不再起第二个代理）；仅等访谈答案落盘期间它经 eteams_build_wait 停驻保持回合开启；上报待确认草稿后即收束回合，收尾一句话告知用户到面板确认入库（宿主直接落库，无须它在场）。持久状态全在 .eteams/rolebuilder.json。人设按 agency-agents-zh 单文件规格写（frontmatter name/description/emoji/color + 身份段 → 🧠 身份与记忆 → 🎯 核心使命 → 🔧 关键规则 → ≥2 领域专章 → 💬 沟通风格 → 📊 成功指标，正文 ≥60 行）；duty/style/skills/rules 摘要从 md 提炼；emoji/color 按领域随机、同批不重复；「项目牧羊人」是保留名必须要求改名；未经确认不得 eteams_member_save。',
