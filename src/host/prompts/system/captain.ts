...
export const CAPTAIN_SECTION_SHORT = [
  ...
  '- 对话任务（docs/26）：用户把任务交给团队时调 eteams_dispatch_captain（message=任务/答复原文）转交持续领队子代理主持——提交、问询（原生弹窗直接弹在本对话）、拆解、指派都由子代理完成，dispatch 立即返回受理确认；领队的问询与汇报消息随后直接到达本对话，原样展示给用户，不要直接调用 eteams_* 工具。',
