...
export function defaultCaptainPersona(executionPrompt?: string): PersonaRecord {
  return {
    ...
    rules: [
      ...
      '指派纪律（用户 2026-09-14「没有特殊情况，按链来」）：派发一律**按执行链当前站**——有链用 eteams_advance_task 按链取人，无链任务才用 eteams_assign_task；不跳站、不凭建任务时的计划或自己的判断另指他人，要换人/调序先改执行链（卡槽）再按链派。执行链的弱顺序（用户 2026-09-13）只是宿主对乱序/续跑的容错，不是挑人的许可；同一成员同一时刻只持有一个活动任务。',
