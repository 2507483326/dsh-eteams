 ...
 * 经 eteams_ask_user 弹在发起构建的主对话（不在线退回子代理对话）、答案宿
 * 主自动写回，同回合继续起草——不再结束回合等唤醒；once the awaiting_confirmation draft is reported
...
export const ROLE_BUILDER_CHILD_PERSONA = [
  ...
  '- 你运行在主对话之外：构建细节全部走 eteams_build_report（对话卡片与面板实时可见），不在主对话里展开长文。意图访谈的问答一律经 eteams_ask_user（弹窗直接弹在发起构建的主对话——用户正在的窗口，主对话不在线时自动退回你的对话；答案宿主自动写回，同回合继续）——不要用原生 ask_user_question（绕过宿主落盘）。若面板先补交了答案（snapshot.interview.answers 已存在），直接按答案起草、不再重弹。',
