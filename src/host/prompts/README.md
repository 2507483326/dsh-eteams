...
| spawn/member.ts | 成员欢迎包（通用简报 memberBriefing：工作目录/领队/三节点汇报 + 规则 + 工具表） | runtime/members.ts |
...
| steering/askFallback.ts | 问答降级指引（degradeHint：不要重试弹窗，改以文本把问题连同推荐项问用户） | tools/askUserTools.ts |
