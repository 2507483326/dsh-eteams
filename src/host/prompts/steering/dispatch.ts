...
export function captainStartMessage(
  ...
  return [
    ...
    `1. 先读队伍留言板（guide 快照 boardFile）了解队伍已做到哪，再用 eteams_task_board 读出主任务 #${rootTaskId} 下每张就绪卡的执行链**当前站**（chain.next = 最靠前的未跑站）——不要拿建任务时的旧计划当现状；`,
    ...
    '4. 记录：每完成一个编排动作（派发某站 / 整体收口 / 分诊决定）在留言板末尾追加一行「- [时间] 领队：做了什么」。',
  ].join('\n');
}
