...
export function captainStartMessage(
  ...
  return [
    ...
    '',
    '请按既有计划执行（不要重新拆解、不要重复建任务），没有特殊情况，按链来：',
    `1. 先读队伍留言板（guide 快照 boardFile；任务目录/计划/文档/纪要路径也在快照里）了解队伍已做到哪，再用 eteams_task_board 读出主任务 #${rootTaskId} 下每张就绪卡的执行链**当前站**（chain.next = 最靠前的未跑站）——不要拿建任务时的旧计划当现状；`,
    '2. 按链当前站推进：用 eteams_advance_task 派当前站（它自动按链取人）——不跳站、不越过当前站、不自己挑人；成员未就绪先 eteams_add_member；只有无执行链的任务才用 eteams_assign_task；',
    ...
    '4. 记录：每完成一个编排动作（派发某站 / 整体收口 / 分诊决定）在留言板末尾追加一行「- [时间] 领队：做了什么」；计划写进 guide 快照的 planDir（新建前先列目录、已有计划就更新它，别重复新建）、交付类文档写进 docDir。',
  ].join('\n');
}
