...
- 团队模式下「任务 / 非任务」判别是一等纪律：简单对话、闲聊、纯问答**直接回应、不为它们建任务**；用户说「启动/开始/继续（项目）」是**开跑指令**——按既有执行链指派，不当作新需求去增补小任务；只有带明确交付物的真新需求才在锚定主任务下增补子任务（用户 2026-09-13 原话「简单对话和启动项目这种不加入新任务」）。落地要点：同一判别要在**所有会读到它的模型入口口径一致**——主会话团队绑定 band（`src/host/prompts/system/sessionTeam.ts` 的有/无领队两分支与红线）、领队子代理提示（`src/host/prompts/spawn/captainChild.ts`）、领队人设（`src/host/prompts/personas/captain.ts`），并各补一条锁定该文案的测试。泛化：提示词纪律类修复不要只改一处，凡是同一规则会被多处模型入口读到的，必须同步到位并断言防漂移。 Confidence: 0.5
...
- 用户以「暂时」措辞要求停用某个交互时（2026-09-13 原话「任务列表卡片暂时去掉拖拽」），实现为**可一行翻转的命名开关常量**而非删代码：如 `const SUBTASK_REORDER_ENABLED: boolean = false`，拖拽源/放置目标的 canDrag/canDrop 前置该开关、把手 JSX 包进条件渲染（关闭时把手不渲染、不高亮），下游状态与提交核（reorderError/submitReorder）保留为恢复挂载位，注释标明日期+用户原话+恢复方式；不动纯逻辑与其单测。泛化：带「暂时/先去掉」字样的需求按可逆停用处理（diff 最小、恢复一行），与「撤功能做整链清理」按用户是否说了「暂时」区分。 Confidence: 0.5
