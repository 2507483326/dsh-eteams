...
- 承接分析结论的「那就……／就按……做」式简短指令即是拍板授权，应直接进入实施、不再复述设计或再要确认（2026-09-11 实证：用户看完 kind/状态分支的完整归因后一句「那就创建中不允许点进去，加上创建中loding效果」即开工，待下一轮直接验收改动与验证结果）——与「反问/质疑不是拍板」互补：表态词（那就/就按/开工）出现才动手。 Confidence: 0.6
- 未就绪的中间态不外露（用户迭代 2026-09-11「创建中不允许点进去」）：任务 status=creating（面板手动创建、待领队/主会话完善收口）时不允许进入详情页——列表卡与看板任务卡整卡禁点（cursor-default、不挂 onClick）、撤「详情」钮，卡身进度计数行换共享加载行（旋转 loader + 「正在完善任务…」，收口 shared/components 供两卡共用），详情页再兜一道 enterable = selected !== null && selected.status !== 'creating' 不满足即 navigate('/tasks')（防持久层 drawerTaskId 恢复的旁路）；「删除」保留，是创建中的逃生门（删不掉就卡死）。 Confidence: 0.6
