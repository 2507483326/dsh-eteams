...
- 用户可见的标识/显示名要以人名（可读文本）开头，不用纯数字/ID 串——数字尾串会被误读成时间（用户 2026-09-12 原话「除了领队，后续派发的子代理上面的名字没有了都是时间显示了」）。机制：子代理 label 就是宿主会话头部的显示名（dsh-client-runtime projectList：`displayTitle = child.label ?? childId`），v7 的 member label `eteams-member:<teamId>:<mainTaskId>:<employeeId>` 是纯数字三元组、在头部读起来像时钟；修为 `eteams-member:<名字>（T<主任务id>-ET<工号>）`——名字在前，工牌后缀后置仍保 (任务, 工号) 作用域、同名成员可区分。泛化：任何呈现给用户的名称/标签优先人类可读语义，作用域/序号类后缀放到后面；此类格式变更要同步更新 pin 旧格式的测试断言与文档/CLAUDE.md。 Confidence: 0.55
