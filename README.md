...
- **面板视觉（docs/22）**：Tailwind 官网风格（官网色板兜底 + Inter var/Fira Code 字体栈 + docs 式宽侧栏，窄面板回落 84px 窄栏）+ 背景板（右上角格子高地图向左下渐隐 + ≤3 辆像素坦克沿网格线缓行；零交互、隐藏页暂停、`prefers-reduced-motion` 静帧）
- **面板视觉二期（docs/23）**：accent 全族 DSW 蓝复用（宿主 `button-info-fill`：亮 `#4176e6` / 暗 `#679efe`，品牌淡底 `--business-tint`）；chrome 控件全面 shadcn/ui 化（Button/Badge/Alert/Progress/Select/Tabs vendored 组件 + Radix 三件套，Select 走自管 portal 容器）；背景板点缀色联动 DSW 蓝
