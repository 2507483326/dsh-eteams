...
### A.5.2 界面草图（对话任务区块；2026-09-05 四轮卡槽下置版）
...
│ │ │ （全边框卡，按执行序展示带序号；卡=拖拽源/目标，  │  │  （七轮 DA20）│
│ │ │   拖 A 卡到 B 卡=调执行顺序——兄弟依赖链改写）     │  │              │
│ │ ┌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌┐              │               │
│ │ ╎ 卡槽（任务行下方独立一行；横向单行，五轮 DA18）    ╎←chip=替换/×=移除│
│ │ ╎ (◉张三)×  (◍李四)×  [＋多选] ←chip拖到chip=调序(六轮)╎ 空白处=追加     │
│ │ ╎ （chips 按序=接力顺序；不设上限，DA15 已废止）    ╎                  │
│ │ └╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌┘              │               │
│ │   （可编辑窗口：TaskStations 行被框替代——框已承整链）    │               │
│ │ ───────────────────────────────────────────────────────  │              │
│ │ 团队成员                                              │               │
│ │ ┌────────┐ ┌────────┐ ┌──────────┐ ┌╌╌╌╌╌╌╌╌┐          │               │
...
拖拽悬停时的卡槽（放大态）：
┌──────────────────────────────────────────────────────────┐
│ (◉张三)×  (◍李四)×  ＋ 拖入成员  （圆角 4px，工号入悬浮提示）│  悬停空白处：实线 + brand 淡底 = 追加
│                  └─悬停 chip：brand 环 + 上浮 = 替换该站
└──────────────────────────────────────────────────────────┘
```
