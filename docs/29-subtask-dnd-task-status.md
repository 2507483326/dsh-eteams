...
│ │ ─────────────────────────    │ └───────────────────────┘ 挪入底栏左  │
│ │ ◔ 待开始  [开始][详情][删除] │ ←底栏每卡常驻：左状态 pill（2px 圆角  │
...
地提示——二十七轮 DA40）；不列小任务明细）
```

**主任务详情页（= 整个任务的编排面；返回条 + 头部卡（二十轮 DA33 收三件 + 二十四轮 DA37 罗列条回卡只留 chips 行）+ 指派提示块（二十四轮 DA37 拆出卡下、二十五轮 DA38 竖线加粗加色）+ 编排全套 + 整体「开始」（二十五轮 DA38））**：
