...
│ │ ─────────────────────────    │ └───────────────────────┘ 挪入底栏左  │
│ │ ◔ 待开始        [详情][删除] │ ←底栏每卡常驻：左状态 pill（2px 圆角  │
...
**主任务详情页（= 整个任务的编排面；返回条 + 头部卡（二十轮 DA33 收三件 + 二十四轮 DA37 罗列条回卡只留 chips 行）+ 指派提示块（二十四轮 DA37 拆出卡下）+ 编排全套）**：
