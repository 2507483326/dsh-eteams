...
| DA46 | 详情页头部卡**去 #id**、编辑态**标题 Input 拉长自适应**、**展开钮/展开区（小任务卡同款）**（2026-09-06 三十三轮拍板） | 用户原话「任务详情页面 把 #1 去掉，然后编辑时 标题input框拉长自适应，和下面的小任务一样加载展开按钮」：①**去 #id**——头部卡信息组撤 `#{task.taskId}`（列表卡 DA25/小任务行头 DA44 之后头部卡对齐；挂靠行 `#{parent.taskId}` 与阻塞 `#{task.blockedFrom}` 引用不动——修 DA25「详情页头部卡的 #id 不动」口径）；②**编辑态拉伸**——信息组条件 `flex-1`（subjectEditor 非空才加，非编辑态零变化；Input 保持 INLINE_SUBJECT_INPUT_CLASS）——标题 Input 拉长占满标题行剩余宽度（短 Input 根因：信息组无 flex-grow，Input 的 flex-1 basis-0% 无处伸展）；③**展开钮/展开区**——TaskHeaderCard 增 editing/expanded/onToggleExpanded 三 optional props（未接线调用位零变化），expandable 判据组件内自足（说明/合同 trim，同 SubtaskItem 口径），箭头 ghost 钮居 actions 簇最右（DA32/DA41 同款类串），展开区 `arrowShow && expanded` 门 + border-t 分区（编辑器/只读正文二选一——DA42 语义同构：编辑先展开、编辑中收起草稿保留、幂等重开、保存后留展开态显新只读内容）；页面态 `headerExpandedId` 单槽 id（详情页一次一张头部卡，切换任务即视为收起） |

| DA47 | 任务详情/小任务展开区只读正文改**单篇 MD 渲染**（说明+合同并读，说明段独换行升格硬断行）（2026-09-06 三十四轮拍板） | 用户原话「任务详情展示怎么不是按照MD展示的，文字都粘一起了」——根因读库实锤：主任务 description 是单换行多行纯文本（\n 分隔无空行），contractMd 为 null，展开区（DA46）说明行纯文本渲染 → HTML 折叠换行全粘一行（合同部分经 MarkdownDoc 渲染正常）：①新 shared `taskBody.ts`——mergedBodyOf 自 taskDetailPage 逐字迁出共用（DA41 说明+合同并读口径不变），新增 **readBodyOf** 只读版：说明段独换行（`/(?<!\n)\n(?!\n)/g`）升格 GFM 硬断行（行尾两空格，remark break → `<br/>`，逐行可见）、空行段落断不动、**合同段逐字透传**（紧凑列表/代码块不吃进升格——升格只作用于说明段）；②头部卡与小任务卡展开区只读分支统一 `<MarkdownDoc text={readBodyOf(task)} />`（说明+合同单篇 MD 文档，与就地编辑器同源；「说明：」标签行随并读撤除）；③只读升格不进编辑器载荷（mergedBodyOf 原样）——保存不回写升格后的行尾空格，无数据污染 |

| DA48 | 成员卡槽拖入成员改**松手放置判位插入**（chip 定点替换废止）+ 悬停插入指示线（2026-09-06 三十五轮拍板） | 用户原话「小任务列表拖拽成员到成员槽不是松手追加，而是松手放置，此外需要判断是放置在成员之前还是之后」：①**判位插入**——成员拖入成员槽由「空白处=末尾追加」改**松手放置**：落点 X 与各站点 chip 中点比距定插入位（左半插该成员**前**、右半插其**后**、全越过=末尾追加、空链=index 0 即追加；纯函数 `insertionIndexOf`（逐中点比距、等中点归右半）+ `chainAfterInsert`（不可编辑/链内同名→null、index 夹取 [0, 链长]）；`nextChainAfterDrop` 删除——末尾追加由判位插入等价承接、chip 定点替换废止）；②**悬停插入指示线**——悬停期间容器内渲染品牌色 2px 竖线（锚落点判位 chip 左缘/末 chip 右缘，offsetLeft 内容坐标滚动稳），落点前后判位可视化；「＋」钮悬停文案交换（松手追加）撤除常显 ＋；③**chip 替换废止**——StationChip drop target 收窄为站点类型只剩调序，chip 上拖成员不再替换该站（替换改走 ×移除 + 再拖入；悬空名修复同走 ×+再拖入），chip title「拖到另一 chip=调序」、容器 title「拖入成员按落点插入（成员前/后）」；④**去重闪现落容器**——成员 drop 全落容器（chip 不再收成员类型），同名 200ms 闪现统一落容器边框，chip 侧闪现撤除 |

| DA49 | **draft 派发即就绪**：开始钮/派发核/接力发棒序放行 draft（2026-09-06 三十六轮拍板） | 用户原话「任务点击开始没有反应」——根因读库实锤：旧库（v2 时期导入）的小任务状态是 draft（建任务即 ready 的现行口径之前），而 DA37 起 draft/ready 面板同显「待开始」、draft 卡又没有晋升钮——①整体开始 `startGroupTask` 发棒序只消费 ready，draft 卡静默 `continue`，返回 {started:0, skipped:[]} 零反馈；②客户端 `isStartable` 只认 ready，draft 卡连开始钮都不渲染——双层闸一起挡死，点开始等于没点。订正：①**派发核 draft 即就绪**——`prepareAssignment`（forReassign !== true 时）开头 draft 状态先 `applyTransition(task, 'ready')`（draft→ready 本就是合法边；依赖未完成/占用等校验在其后原样兜——draft 卡依赖未完成会以「依赖未完成：N」诚实跳过而非静默吞），状态闸改为只挡非 ready 非 draft；②**发棒序放行 draft**——`startGroupTask` 循环过滤 `['draft','ready']`（draft+空链卡照旧「需要选择成员」跳过、依赖未完成照旧按卡透出原因），单任务开始/领队指派/接力续派同走 prepareAssignment 一并生效；③**客户端判据放行 draft**——`isStartable` = ready ‖ draft（DA37 待开始语义自此闭环：面板「待开始」的卡一定渲染开始钮/「需要选择成员」提示面），小任务卡/任务详情页/组卡判据收拢位零散改判自动生效；④已入执行（wait/paused/wait_decision/wait_user/终态）依旧不渲染不派发，无链 draft 卡行内提示「需要选择成员」不变 |
| DA50 | **主会话锚点冷恢复**：起会话/接力不再要求主会话窗口在线（2026-09-06 三十七轮拍板） | 用户原话「还记得那个会话的ID吗？不能直接跳转到那个会话吗」「那就可以直接跳到这个会话启动任务啊」——根因：锚点解析只有两级（领队行登记的主会话在册 → 客户端心跳在册），成员第一次起会话必须拿活的父 Agent 当 parent（startContinuable/followup 都收 Agent 对象不收 ID 串），两级都落空时哪怕领队行记得主会话 ID 也只能报错，逼用户先手动开窗口。而 DSH 运行时 `AgentRegistry`（ctx.agents 真实接口）本就提供 `resume({ resumeSessionId })`——按持久化会话 ID 加载会话回活代理（不跑任何回合，只挂进注册表）。订正：**captainFor 升三级梯度**——①领队行锚点在册 → ②心跳锚点在册 → ③两锚都不在册但领队行记得 ID 时冷恢复（feature-detect `agents.resume`，恢复成功即返回句柄里的活 Agent；恢复失败/旧运行时无此 API 落 undefined 走原报错）；恢复的会话与领队行同 ID，ensureSpawned 重锚是 no-op；句柄 dispose 绝不调用（create 语义里 dispose 拆会话，复活的主会话像用户开着的窗口一样常驻到进程回收）；ensureSpawned 报错文案对齐「主会话锚点不可用（不在线且无法冷恢复）」；恢复成败各落一条 warn 日志。面板/对话指派、整体开始、接力续派、resumeTask 四路同走 captainFor 一并生效 |
...
| `chainCursor === -1` 且 draft/ready（任意链长） | 拖入成员 = **松手放置判位插入**（三十五轮 DA48：落点 X 与各站点 chip 中点比距——左半插该成员**前**、右半插其**后**、全越过=末尾、空链=index 0 即追加；不设上限——DA15 曾限 2 站，四轮废止；链内同名即 DA8 no-op 不发请求，200ms 闪现落容器）；**站点 chip 拖到另一站点 chip** = 调序（六轮 DA19：被拖站搬移到目标位、stageBrief 随站走，自拖自放 no-op）；**「＋」点击** = 成员多选面板（六轮 DA19：勾选若干人按勾选顺序末尾追加；二十二轮 DA35 起链中成员不进列表） |
...
> 纯函数签名（三十五轮 DA48 起）：`chainAfterInsert(task, member, index)`——判位插入：不可编辑 / 全链同名去重 → null（no-op + 200ms 闪现落容器），index 夹取 [0, 链长]（快照中途变化防越界），新站 stageBrief 空串、其余站原值保留；`insertionIndexOf(midpoints, x)`——落点 X 对各站点 chip 中点比距（x < 中点 → 插其前、等中点归右半、全越过 → 末尾、空数组 → 0），中点由 taskAssign 的 `stationMetricsOf` 供（chip 中点视口坐标，与 `monitor.getClientOffset` 同系）。`nextChainAfterDrop` 删除——「空白处=末尾追加」由判位插入空链/末尾位等价承接、「chip 定点替换」废止。六轮新增：`chainAfterReorder(task, fromIndex, toIndex)`（from===to / 越界 / 只读 → null）、`chainAfterAppendMany(task, members)`（按勾选顺序追加，过滤链内已存在名字，一个都不新增 → null）。七轮新增：`executionOrderOf(tasks)`（兄弟依赖拓扑展示序）与 `depPatchesForReorder(tasks, from, to)`（拖卡 → 依赖改写补丁，A.3.4）。
...
### A.5.1 成员卡槽（TaskAssignDropBox；2026-09-04 二轮按 DA13/DA14、2026-09-05 三/四/五/六轮按 DA15（废止）/DA16/DA17/DA18/DA19 修订、二十二轮按 DA35 修订、三十五轮按 DA48 修订）
...
| 拖拽悬停（容器空白处） | `canDrop && isOver({shallow:true})`：虚线转实线 + `border-primary` + 品牌淡底 `bg-[color:color-mix(in_srgb,var(--primary)_12%,transparent)]`（二十三轮 DA36：原中性 pill 底与 chip 底同色、chip 列表顶满时高亮看不出，用户拍板「拖拽时高亮显示被遮挡了」），文案/提示指向「追加」（仅成员类型——容器不收站点类型，六轮 DA19）；**三十五轮 DA48 修订**：悬停判位可视化——容器内渲染品牌色 2px 插入指示线（锚落点最近 chip 中点左缘/末 chip 右缘，offsetLeft 内容坐标滚动稳）、「＋」钮悬停文案交换撤除（判位由指示线承担），松手按落点插入成员前/后（不再恒末尾追加；chip 上悬停也落容器） |
| 拖拽悬停（chip 上） | 该 chip `ring-1 ring-inset ring-primary` brand 环（box-shadow 不引发回流；二十三轮 DA36 加 ring-inset——环画进 chip 内缘，滚动容器 overflow-x-auto 滚动态裁不掉外缘环，同上用户拍板）+ 上浮阴影（站点 chip 拖到其上=调序，六轮 DA19；**三十五轮 DA48 修订**：chip 不再收成员拖入——成员拖到 chip 上按左右半判位插入、高亮与插入指示线落容器，「chip 上=替换该站」口径废止，替换改走 ×移除+再拖入）；容器不追加高亮（shallow 区分；DA48 后成员拖拽悬停高亮全落容器） |
...
**三十三轮 DA46（2026-09-06）修订（详情页头部卡去 #id + 编辑态标题 Input 拉长 + 展开钮/展开区（小任务卡同款））**：用户原话「任务详情页面 把 #1 去掉，然后编辑时 标题input框拉长自适应，和下面的小任务一样加载展开按钮」——①头部卡信息组撤 `#{task.taskId}`（列表卡 DA25/小任务行头 DA44 之后头部卡对齐；挂靠行 `#{parent.taskId}` 与阻塞 `#{task.blockedFrom}` 引用不动）；②信息组编辑态条件 `flex-1`（subjectEditor 非空才加，非编辑态零变化）——标题 Input（INLINE_SUBJECT_INPUT_CLASS）拉长占满标题行剩余宽度（小任务行头 Input 未动——用户未点名，留可选跟进）；③头部卡增展开钮/展开区（与小任务卡同款）：TaskHeaderCard 增 editing/expanded/onToggleExpanded 三 optional props，expandable 判据组件内自足（说明/合同 trim，SubtaskItem 同口径），箭头 ghost 钮居 actions 簇最右（DA32/DA41 同款类串），展开区 `arrowShow && expanded` 门 + border-t 分区（编辑器/只读正文二选一，DA42 语义同构：编辑先展开、编辑中收起草稿保留、幂等重开、保存后留展开态显新只读内容）；页面态 headerExpandedId 单槽 id（详情页一次一张头部卡，切换任务即视为收起）；同轮 dedupe：taskDrawer TaskDetailContent 撤合同区（ContractMd 组件/用例行/MarkdownDoc import）——合同/说明展示与编辑归头部卡展开区，避免同页双份合同（详情正文区只承阻塞/状态说明/幂等说明/产出/时间线）。

**三十四轮 DA47（2026-09-06）修订（展开区只读正文改单篇 MD 渲染）**：用户报「任务详情展示怎么不是按照MD展示的，文字都粘一起了」——根因读库实锤：主任务 description 单换行多行纯文本（无空行），DA46 展开区说明行纯文本渲染 HTML 折叠换行全粘一行；订正：新 shared taskBody.ts（mergedBodyOf 自 taskDetailPage 迁出共用 + readBodyOf）——说明+合同并读**单篇 MD 文档**（与就地编辑器同源，「说明：」标签行撤除），说明段独换行升格 GFM 硬断行（remark break → `<br/>`，逐行可见；空行段落断不动；合同段逐字透传，紧凑列表 `1. a\n2. b` 不吃进升格）；升格只在只读展示，编辑器载荷走 mergedBodyOf 原样，保存无数据污染。头部卡与小任务卡展开区同款。

**三十五轮 DA48（2026-09-06）修订（成员卡槽松手放置判位插入）**：用户报「小任务列表拖拽成员到成员槽不是松手追加，而是松手放置，此外需要判断是放置在成员之前还是之后」——订正：成员拖入成员槽由「空白处=末尾追加」改**松手放置判位插入**（落点 X 与各站点 chip 中点比距：左半插该成员前、右半插其后、全越过=末尾、空链=index 0 即追加；悬停期间容器内品牌色 2px 插入指示线可视化落点判位，「＋」钮悬停文案交换撤除）；**chip 上拖成员=替换该站（二轮 DA13 口径）随之废止**——放置语义统一，替换改走 ×移除 + 再拖入（悬空名修复同走 ×+再拖入），StationChip drop 收窄为站点类型只剩调序、去重 200ms 闪现落容器。判位纯函数 `insertionIndexOf`/`chainAfterInsert`（11 条规则锁），`nextChainAfterDrop` 删除。
...
| 三十三轮（2026-09-06，DA46 详情页头部卡去 #id + 编辑态标题 Input 拉长 + 展开钮/展开区（小任务卡同款） + 抽屉合同区 dedupe） | `taskHeaderCard.tsx`：①信息组撤 `<strong>#{task.taskId}</strong>`（挂靠行/阻塞引用不动）；②信息组 className 改 cn 条件合并 `subjectEditor !== undefined && 'flex-1'`（编辑态标题 Input 拉长占满剩余宽度，非编辑态零变化）；③新 props editing/expanded/onToggleExpanded（全 optional——未接线调用位零变化）+ 组件内自算 expandable（说明/合同 trim，SubtaskItem 同口径）+ arrowShow = 接线 && (expandable \|\| editing)；actions 簇判据 (actions !== undefined \|\| arrowShow)，箭头 ghost icon Button 居簇内最右（h-6 w-6 flex-none py-0 剥焦点环同 DA41 口径，ChevronDown transition-transform + expanded rotate-180，aria-expanded）；展开区 `{arrowShow && expanded && <div className="mt-1.5 border-t border-solid pt-2">}`（editing && editor 非空 ? editor : 说明行 + MarkdownDoc 合同 MD）——原 {editor} 无条件槽位撤除；`taskDetailPage.tsx`：④新状态 headerExpandedId: number \| null（单槽 id，expandedSubIds 之后——详情页一次一张头部卡，切换任务即视为收起）；⑤openInlineEdit 幂等分支与主分支各加 else setHeaderExpandedId(t.taskId)（「编辑」先展开）；⑥组页/任务页两处 TaskHeaderCard 调用接 editing/expanded/onToggleExpanded 三 props；`taskDrawer.tsx`：⑦TaskDetailContent 撤 ContractMd 函数 + 用例行 + MarkdownDoc import（合同展示归头部卡展开区，避免同页双份；正文区只承阻塞/状态说明/幂等说明/产出/时间线）。无宿主/api/tests 变更；四绿门全绿（27 文件 363 用例——工作树含用户未提交 host 侧测试，高于基线 337 无一失败） |

| 三十四轮（2026-09-06，DA47 展开区只读正文单篇 MD 渲染 + 说明段独换行升格硬断行） | 新文件 `taskBody.ts`：①mergedBodyOf 自 taskDetailPage 逐字迁出共用（DA41 编辑器并读口径不变）；②readBodyOf 只读版——说明段独换行 `/(?<!\n)\n(?!\n)/g` 升格 GFM 硬断行（行尾两空格）、空行段落断不动、合同段逐字透传（紧凑列表不吃进升格，升格只作用于说明段）；`taskDetailPage.tsx`：③本地 mergedBodyOf 撤除改引 taskBody（编辑器载荷 setInlineBody(mergedBodyOf(t)) 不动）；`taskHeaderCard.tsx`/`taskSubtaskItem.tsx`：④展开区只读分支统一 `<MarkdownDoc text={readBodyOf(task)} />`（「说明：」标签行撤除、LINE_CLASS 撤除）。无宿主/api/tests 变更；四绿门全绿（28 文件 369 用例——工作树含用户未提交 host 测试与并行重构，高于基线无失败） |

| 三十五轮（2026-09-06，DA48 成员槽松手放置判位插入 + 悬停插入指示线 + chip 定点替换废止） | `taskAssignCore.ts`：①新增 `insertionIndexOf(midpoints, x)`（逐中点比距：x < 中点插其前、等中点归右半、全越过=末尾、空数组=0）；②新增 `chainAfterInsert(task, member, index)`（不可编辑/链内同名→null、index 夹取 [0, 链长]、splice 插入 stageBrief 空串其余保真）；③`nextChainAfterDrop` 删除（grep 全仓确认代码消费面仅 taskAssign.tsx 与 tests）；`taskAssign.tsx`：④容器 drop 改判位插入（drop 期 `stationMetricsOf` 测 `[data-station-index]` chip 中点（视口坐标）对 `monitor.getClientOffset().x` 判位；测不到容器/落点退末尾追加；`didDrop` 守卫保留）；⑤collect 补 offset + 悬停插入指示线（`INSERT_CARET_CLASS` absolute 2px 品牌竖线，锚 `lefts[i]-1` / 末 chip `offsetLeft+offsetWidth+2` 内容坐标；`BOX_MULTI_CLASS` 加 relative；容器节点经合并 ref 回调存 `boxEl` state——react-hooks 禁渲染期读 ref，caret 渲染期读 state）；⑥空框分支同走判位路径（mids 空→0 与原追加等价，「松手追加/＋ 拖入成员」文案保留）；⑦有链分支容器 title「拖入成员按落点插入（成员前/后）」、＋钮常显 ＋；⑧StationChip accept 收窄 `STATION_DRAG_TYPE` 只剩调序、useFlash200 撤除、根 div 加 `data-station-index={index}`、title「拖到另一 chip=调序」；⑨就地修一处 DA13 旧残留注释（「chip 级闪现」→「flash 只落容器」）；`tests/taskAssign.test.ts`：⑩原 nextChainAfterDrop describe（7 例，含 chip 定点替换 3 例随语义废止）改写为 `insertionIndexOf` 5 例 + `chainAfterInsert` 6 例。四绿门全绿（28 文件 **373 用例**——基线 369：撤 7 增 11 净 +4，含用户未提交 host 测试无失败） |
