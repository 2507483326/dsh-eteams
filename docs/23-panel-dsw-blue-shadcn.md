...
| D21g | 背景板可见性重定标（R2 用户验收反馈） | 一期「palette 预烘焙 alpha × 使用位 fade 系数再乘 × 画布 opacity 0.5」三重叠乘——网格有效 alpha ~0.001（不可见）、坦克车身 ~0.16（弱成点）。修复：①调色板只带**实色基底**（label/brand），alpha 在使用位**单次烘焙**为最终有效值（结构性消灭叠乘类缺陷）；②画布全局 opacity 1.0（COMPOSITE_ALPHA_CAP=1，废除 0.5 减半层；「不喧宾夺主」改由元素级上限保证）；③有效 alpha 定标：网格 0.07（官网 0.04 档 + 宿主中性色较浅补偿）、高地图 0.11×height（线性，废除 height² 压暗）、峰顶 DSW 蓝 0.13×peak、坦克车身 0.45/深档 0.55/DSW 蓝点缀 0.62；④sprite 重画为 7×5 经典俯视微坦克（左右履带柱 + 三行连贯车体 + 右伸炮管 + 蓝炮塔心，25 实体像素）、像素尺寸 cell/10（3px/像素，全车 21×15px）；⑤新增 2 个可见性回归锁单测（右上网格段 alpha ≥0.06；坦克三档 alpha 落可辨识带 + ≥25 像素 + 3px/像素）。D20e 原「全局 0.5 + 各元素 ≤0.05/0.07/0.4 烘焙值」语义就此废止，硬指标以**有效值**重述 |
...
| R2 修复轮（D21g） | eb3d404 | 用户视觉验收反馈「看不到格子 / 坦克是三个点」：根因 = alpha 三重叠乘（有效 ~0.001/0.16）。修复 = 实色基底 + 单次烘焙 + 画布 opacity 1 + 有效值定标（网格 0.07 / 高地图 0.11 线性 / 峰顶 0.13 / 车身 0.45 / 深档 0.55 / 点缀 0.62）+ 7×5 像素坦克重画（25px 轮廓，3px/像素）；新增 2 可见性回归锁（125 例全绿）；预览再生成（IIFE 15.75KB + 画布 opacity 1 + 136 class 复核 0 缺失） | 4,168,473 B |
| R2 补强（渲染自查） | （本次） | Node 光栅化自查引擎输出（PNG + ASCII 取证）：网格线实测对比度 11.8–13.6/255（可见 ✓）、地形斑驳 sd 2.29 ✓、渐隐方向 ✓；**发现并修复首版 sprite 旋转退化**（履带误画为左右整列 → dir 旋转后呈工字梁）——重画为「行进方向两侧履带 + 中央车体 + 前伸炮管 + 蓝炮塔」，四向 ASCII 终验均成立；新增四向剪影形状锁（126 例全绿，含 25 像素预算修正） | 4,168,727 B |
