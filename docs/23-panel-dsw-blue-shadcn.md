...
| R2 修复轮（D21g） | （本次） | 用户视觉验收反馈「看不到格子 / 坦克是三个点」：根因 = alpha 三重叠乘（有效 ~0.001/0.16）。修复 = 实色基底 + 单次烘焙 + 画布 opacity 1 + 有效值定标（网格 0.07 / 高地图 0.11 线性 / 峰顶 0.13 / 车身 0.45 / 深档 0.55 / 点缀 0.62）+ 7×5 像素坦克重画（25px 轮廓，3px/像素）；新增 2 可见性回归锁（125 例全绿）；预览再生成（IIFE 15.75KB + 画布 opacity 1 + 136 class 复核 0 缺失） | 4,168,473 B |
