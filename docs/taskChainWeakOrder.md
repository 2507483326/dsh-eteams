...
主任务目录根下一块 `留言板.md`（用户 2026-09-15 扁平化后路径为 `teams/<主任务号>-<slug>/留言板.md`——一个主任务一个目录、团队名不进路径），由宿主在文档树物化时 **create-only** 落盘（`renderTeamDocs`）；同一主任务的**领队与全部成员共用一块板**（小任务成员经 `boardFileAbs` 上溯到主任务目录）。
...
### 4.3 任务目录扁平化（用户 2026-09-15）

**一个主任务一个目录** `teams/<主任务号>-slug/`，主任务与其全部小任务共用（去掉 `tasks/`、`sub/` 与团队层——一块留言板 / 一个计划夹 / 一个文档夹是「一个主任务」的治理面）。目录下固定四样，路径全部由宿主算好下发（成员简报给工程根 / 任务目录 / 留言板 / 本任务纪要 / 文档目录；guide 快照给 taskDir / boardFile / planDir / docDir / minutesFile）：

| 物 | 名字 | 语义 |
| --- | --- | --- |
| 留言板 | `留言板.md` | create-only，领队 + 全员共用一块 |
| 纪要 | `<任务号>-slug.纪要.md` | 每任务一份（含主任务）：上半段宿主幂等渲染的合同视图（替代 `contract.md`），下半段 `## 纪要正文` 由模型写（替代 `notes.md`） |
| 计划 | `计划/` | 宿主只建目录，文件名与份数自由（各 agent 自建，宿主不去重） |
| 文档 | `文档/` | 文档类产出（交付说明 / 验收结论 / 调研笔记）扁平放，文件名自由 |

- 代码产出写**工作区根**（它就是这些任务共同服务的工程）；治理文档写任务目录。
- 防重复：路径由宿主下发 + create-only + slug 只由宿主 `taskSlug()` 生成 + 纪要名带任务号；主题改写时宿主改目录名并同步改该份纪要文件名（同号旧名先改名，不留重复文件）。
- **存量旧布局不迁移、不兼容**：物化只处理 work_dir 与当前规则相符的主任务（`isCurrentLayout`），旧目录冻结不动。
- 团队 `README.md` 撤除；小任务不再分配 `work_dir`（`taskDirRel` 上溯主任务），指派 / 改名 / 删除都不动目录。
