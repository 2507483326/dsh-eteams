...
> 面向开发者的深度说明：命令总览、工具面清单、项目结构、运行时主干、存储设计与命名规范。
> 面向插件市场的概览见 [README.md](../README.md)。
...
- **Web 面板（浏览器端）**：宿主回环路由 `/eteams-api` 前缀下读端（全量快照 / 事件增量 / 执行线路 / 成员对话框 / 用量日历）+ 写端（session-team、session-persona、client-log 等）；面板壳内 MemoryRouter 四页签：看板 / 团队 / 角色 / 任务；轮询 1s 活跃 / 5s 空闲 / 页面隐藏暂停 / 恢复即刷。
...
src/client/             浏览器端插件
  ...
  pages/                页面（二级域目录 board / roster / tasks / team，teamsView 只留壳：index/rail/routes）
...
- **任务状态机**：8 态收敛（creating / ready / start / wait / paused / wait_user / completed / cancelled），非法转移抛 `TransitionError`（带可执行中文提示）；原 `draft` / `wait_decision` / `failed` 已并入 `ready` / `wait_user`；「阻塞」不设独立状态，依赖不拦截派发、只作排布提示。
...
- **Schema v17，16 张表**：`schema_meta`（元数据键值）、`team`、`roles`（角色库）、`team_members`（班底）、`task`（大/小任务，含合同 MD、成员链、游标、重试计数、状态说明、阻塞还原点、**目录归属两列 `work_dir`=当前会话目录（建任务时冻结，绝对）/ `task_dir`=任务目录（相对 work_dir）**）、`task_members`（执行实例，领队行 `name='团队领队'` 且 `main_task_id` 为空）、`attempts`（一行一次尝试，含一次性 token、进度、结果、变更文件）、`events`（追加审计事件）、`mail_messages`（邮箱）、`decisions`（升级决策）、`task_status_changes`（状态流转记录）、`usage_detail` + `usage_daily_total`（用量明细与日总计，DB 即唯一存储）、`ask_questions`（问答弹窗）、`capability_gaps` + `routing_memos`（子代理能力缺口与常设路线）。
- **任务工作文档**（用户 2026-09-15 扁平化）：`<任务 work_dir>/teams/<主任务号>-slug/` —— 一个主任务一个目录（主任务与其全部小任务共用；`work_dir` 建任务时冻结，所以同一任务不会在别的会话工作区多出一份），下 `留言板.md`（create-only，领队+全员共用一块）+ 每任务一份 `<任务号>-slug.纪要.md`（上半段宿主幂等渲染的合同视图、下半段纪要正文）+ `计划/`、`文档/` 两个只建目录、内容自由的夹。旧布局（`tasks/`、`sub/`、`contract.md`、`notes.md`、团队 `README.md`）不迁移、不兼容。
- **schema 副本约束**：[schema.sql](src/host/state/schema.sql) 与 `src/host/state/db.ts` 内嵌 `SCHEMA_SQL` 常量必须逐字一致（宿主是单文件 bundle 读不到同目录资产，故内嵌一份；schema.sql 是审核对照副本）。版本迁移在 `getDb` 内按 `PRAGMA table_info` 守卫逐条 ALTER + 回填（幂等，重开自愈）；数据无法安全推导时**只补形状、不回填**（v17 `task.task_dir` 先例：旧行两列不齐即判旧布局、不参与物化）。
