...
- **意图访谈经会话流转（docs/19.16）**：后台代理调 ask_user_question 用户根本看不到（选项框只落在发起会话）——改为**会话内问卷**：`BuildReport.interview` 把问题写入会话（`InterviewState{questions,answers,answeredAt}`，`answerBuildInterview` 落答案），工作台构建中分支渲染可点选问卷（单选/多选、「提交回答」），会话卡片加「✍️ 意图访谈待作答」徽标；宿主 `POST /rolebuilder/interview` 存答案并经 `subagents.followup` 把逐题答案发回构建代理（「用户已完成意图访谈…」）继续起草。构建代理纪律同步改写：访谈必须走 interview 参数、发布后结束回合等答案、禁止调用 ask_user_question（后台发起用户不可见）；仅降级路径（主会话亲自构建）保留选项框。
- **轻量调度（docs/19.16）**：领队只做「查一次 + 派发」两个 tool call——`eteams_team_status` 新增「构建会话」行（status · step · 草稿名 · 秒龄），非空且 active/awaiting = 已有构建进行中，只回提示不派发；否则直接 subagent 派发。开会话、查重、访谈、起草全部下沉子代理：其首播报必须 `newBuild=true` 开会话，**卡片在子代理首次播报后才出现**（`/eteam` 处理器不再预开会话）。派发时会话尚不存在 → childId 先写旁路文件 `rolebuilder-agent.json`（`setBuildAgentId`），子代理首播报开会话时由 `stampAgentRef` 合并进会话并消费；`builderChildRef` 统一为 cancel/resume/interview 解析 child（会话 agentId 优先，旁路文件兜底）。`/eteam` 处理器加同款门禁：已有构建进行中时 steer 提示、不派发。
