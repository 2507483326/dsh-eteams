...
- **自动跳转让位**：改为**时间戳判定**——capture 监听记录用户最后一次点击时间 `lastUserClickAt`，自动跳转仅当 `lastUserClickAt < startedAt`（会话开启后用户没点过任何东西）才允许。与宿主 tab 的标记结构完全解耦（`button[role=tab]` 与旧版 leaf div 两种变体，按选择器匹配会漏——漏掉的点击之后，受理竞态窗口内的重试跳转就会闪跳拽人）。发送动作发生在会话创建之前，不影响跳转资格；`openMemberBuilder` 的程序化点击落在跳转之后无影响。另修 `pendingGotoAdd` 标记泄漏：已挂载面板的事件路径也消费标记，防止滞留到下次挂载误拽。
...
- **放弃即静默（docs/19.16，已被一次性阶段制取代）**：曾以 interrupt+毒丸收尾持久代理——随阶段制改造删除。
- **意图访谈经会话流转（docs/19.16）**：后台代理调 ask_user_question 用户根本看不到（选项框只落在发起会话）——改为**会话内问卷**：`BuildReport.interview` 把问题写入会话（`InterviewState{questions,answers,answeredAt}`，`answerBuildInterview` 落答案，仅 active 会话可答），工作台构建中分支渲染可点选问卷（单选/多选、「提交回答」），会话卡片加「✍️ 意图访谈待作答」徽标；提交后宿主派发阶段 B 一次性代理继续起草。构建代理纪律同步改写：访谈必须走 interview 参数、发布后结束本阶段、禁止调用 ask_user_question（后台发起用户不可见）；仅降级路径（主会话亲自构建）保留选项框。
- **轻量调度（docs/19.16）**：领队只做**一个 tool call**——`eteams_build_dispatch(request=原文)`：自带门禁（已有构建进行中返回 busy，回提示不派发），否则派发阶段 A 一次性代理，卡片在其首次播报后出现。开会话、查重、访谈、起草全部下沉阶段代理；`/eteam` 处理器同款门禁（busy 时 steer 提示）。
- **一次性阶段制（docs/19.16 终稿）**：不再使用持久（continuable）子代理——`.eteams/rolebuilder.json` 本就是持久状态（docs/09 磁盘是唯一真相），构建拆为三个**一次性阶段代理**，每个干完自然结束回合，不留下任何可续聊记录：阶段 A（受理：newBuild 开会话 → 查重 → 发布意图访谈）由 `eteams_build_dispatch` 工具或 `/eteam` 处理器派发；阶段 B（起草 → 深化 → 完整草稿 awaiting_confirmation）由 `POST /rolebuilder/interview` 存答案后宿主派发；阶段 C（放弃后继续）由 `POST /rolebuilder/resume` 翻转状态后宿主派发。阶段 prompt 由 `builderPhases.ts` 从会话快照（request/stepsDone/draft/访谈逐题答案）现场组装，persona = `ROLE_BUILDER_CHILD_PERSONA`（一次性阶段制纪律），toolFilter 同前。由此删除了整套持久代理机关：agentId 旁路文件、cancel 的 interrupt+毒丸、resume/interview 的 followup——cancel 只翻状态，进行中的一阶段代理最多自然跑完本阶段（终态守卫挡住一切后续写入）。派发时 `setBuildParentSession` 记住主会话 id（`rolebuilder-parent.json`），供 interview/resume 路由解析父 Agent 归属后续阶段。注意：harness 的 `subagent` 工具派生的是可续聊持久子代理——调度员纪律已禁止用它派发构建。
- **空会话唤醒（用户反馈 2026-09-04）**：斜杠命令只落 `command/run`/`command/done` 两条 log-only 记录——既不是 prompt 也不开 turn，宿主的 blank→engaged 相位机（blank 仅由 `turn/start` 翻转；命令、plan/goal 等插件事件都不开 turn）永远不翻：空白主窗口发 `/eteam` 后，命令执行、阶段代理照常开跑，但主窗口仍停在未开始屏——对话视图渲染不出，命令节点与构建卡片不挂载，卡片的「发送即跳转」无从发生，构建全程隐形（用户视角：「子 agent 创建了，界面却没进对话」）。修复：`/eteam` 处理器受理派发后，若接收会话从未开过 turn（`agent.session.events` 无 `turn/start`，与宿主 `sessionBlank` 同口径），steer 一条 plugin notice（form `notice`，摘要「成员构建已受理——本会话无需操作」）让 idle driver 立即开 turn——宿主 running 位翻转 → 客户端 blankBit 清零 → 对话视图出现 → 命令节点渲染 → 构建卡片挂载并按 docs/19.16 的时间戳判定自动跳转到新增工作台。开过 turn 的会话不加这一回合（无应答噪音）；steer 失败只记日志不外溢，构建照常后台进行。
- **继续构建拒绝要可见（用户反馈 2026-09-05「点继续构建没反应」）**：恢复的宿主前置校验（父会话在线性）被面板吞错——`membersTab` 的 resume 此前 `catch {}` 静默，宿主对「父会话不在线」等情形回 409/400（诚实拒绝、不改状态，防止恢复成 active 却没有代理续跑），但用户侧零反馈，看起来像按钮死了。修复：拒绝原因亮在「已放弃本次构建」卡内（destructive 错误面，与访谈提交失败同口径）——典型文案「父会话（…）当前不在线——构建未恢复。请先打开发起 /eteam 的对话，再点『继续构建』」；恢复成功路径不变（状态翻转 → 轮询刷新 → 工作台接管）。面板路由测试补齐：父不在线 409 且会话保持 cancelled、父在线 200 翻 active 且草稿上下文保留。
- **AI 创建页第二批迭代（用户反馈 2026-09-05）**：四项——① **父会话在线性前置显隐**：`GET /rolebuilder` 在会话存在时附挂 `parentOnline`（读 `rolebuilder-parent.json` + `agents.get` 实时判定，只挂响应不落盘）；「已放弃本次构建」卡仅在父会话在线时渲染——父不在线时继续构建无从派发，进页检测到就不显示，而不是点了才报错（上一批的卡内错误面保留，兜轮询间隙的竞态）。② **填充/复制两钮常驻**：AI 创建页填充成功（`aiPrefill==='set'`）不再收走按钮行——此前 `aiPrefill === 'set'` 分支只剩提示文字，点「填充」两钮齐消失；改为常驻按钮行，已填充态「填充」钮改标「重新填充」，复制随时可用。③ **复制带反馈**：`writeClipboard` 回传是否真写入剪贴板——成功亮「✓ 已复制」、失败亮「复制失败」各 2 秒（ref 持计时器，重复点击先清旧的），不再点了零反馈。④ **「对话页看进度」按钮删除**：新增页头不再提供跳对话视图入口（`activateConversationTab`/`MessageSquare` 引用随按钮删除），构建进度以工作台轮询为准。
- **自动跳转让位锁存前移（用户反馈 2026-09-05「点了对话又跳回团队了，再点击才能跳回对话页面」＝ 19.16 老毛病复发）**：接管判定（startedAt 之后点过任何东西就不拽人）的点击锁存原先挂在 buildCard 模块顶层——但该模块要等对话里**第一张构建卡片挂载**才首次求值。「刚创建」时序恰好倒着走：用户点进对话 → 空会话唤醒的 turn 开启 → 命令节点渲染 → 卡片模块此刻才诞生，此前那次点击记不上，`lastUserClickAt` 恒 0、恒小于 `startedAt`，判定放行，卡片按设计执行一次性跳转，把刚到对话的用户又拽回团队；第二次点击时监听已挂上、`jumpedSessionAt` 也已记档，才稳住。修复：锁存与判定查询迁到 `bridge.ts`（bundle 装载即求值，模块级 `document` capture 点击监听，`__eteamsClickLatch__` 守卫防双份），导出 `userClickedSince(since)`；buildCard 的跳转条件改读该函数（`!userClickedSince(s.startedAt)`），卡片模块无论多晚诞生，bundle 加载以来的一切点击都算数。
- **意图访谈去工作台化（用户反馈 2026-09-05「访谈怎么放到角色创建页面去了」）**：意图访谈的作答只走主会话——构建代理发布访谈后，宿主 steer 主代理用 `ask_user_question` 把问题逐题弹在选择框里（本就在主会话内），用户答完主代理调 `eteams_interview_answer` 提交、阶段 B 续跑；`POST /rolebuilder/interview` 与该工具同一落盘出口（同 startedAt 去重），保持为合法写路径。此前工作台「构建中」分支渲染了一整套平行问卷（单选/多选/提交回答，走 `build/submitInterview`），与对话选择框两套入口并存，用户不知道在哪答、也不知道为什么创建页里也能答——现撤除：工作台访谈态只留提示块「意图访谈——到主会话作答」（问题已发到发起 /eteam 的对话，回到主会话在弹出选择框里逐题作答；没看到选择框可点「重启代理」重新出题；重启失败仍亮 destructive 错误面）。随之删除 `interviewPick`/`togglePick`/`submitInterviewAnswers` 及 `BORDER_L1_CLASS` 引用；dva `build/submitInterview` effect 与 api 通道保留（宿主路由仍在，作休眠备份写路径）；对话卡片文案同步从「点开回答后自动续跑」改为「在本会话作答后自动续跑」——卡片本就在主会话里，点开只会进工作台，那里已无可答的问卷。
