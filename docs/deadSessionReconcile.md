...
**状态：已实现（只在启动时对账：立即 1 次 + 启动后 1/3/6/10s 快扫；不做常驻轮询 + 6 条回归用例）。**
...
```
启动对账（立即 1 次 + 1/3/6/10s 快扫；timer 全部 unref；之后不再排期）
...
- **启动首轮就可能空跑**：apply 时刻 `workspaceRegistry` 往往尚未就绪，
  `collectRoots` 返回空（实测宿主 12:38:57 启动、对账 12:39:26 才生效，正是干等了
  一整轮）。故启动后按 `BOOT_SWEEP_DELAYS_MS`（1/3/6/10s）补几轮快扫，注册表一
  就绪即生效（用户 2026-09-18「扫描的速度好像有点慢」）。
- **不做常驻轮询**（用户 2026-09-18「那个轮询感觉好像没有必要，去掉吧」）：进程内
  「用户点停止 / 回合异常结束」都有 `turn/end` 事件、由中断观察者实时处置；对账只
  负责重启后**没有事件可依**的那批残留，启动跑一次即可。代价是「进程内事件漏路由」
  （注册表查不到会话 → 事件被丢弃）这类漏网情况没有兜底，只会在下次重启时被清掉。
