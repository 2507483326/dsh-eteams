...
> 导入日期：2026-09（由领队会话批量导入）　成员库现状：**2 名系统成员**（见「2026-09 默认角色精简」）
...
- （历史）2026-09 曾批量导入 **186** 个新角色到 eTeam 成员库（库内一度合计 192）。
- 同日按用户要求精简默认角色：成员库只保留 **项目牧羊人**（领队）与 **角色构建师**，见下节。
- 导入成员的 `role` 字段即其**部门分类**（如「工程部」「营销部」），`duty/style/skills/rules` 为从手册提炼的摘要，`personaMd` 为完整角色手册——文件镜像中仍可查。
- 加入团队时用 `eteams_add_member` 按中文名点名即可（需先入库：走 `eTeam --add-people` 或面板「新增角色」）。

## 2026-09 默认角色精简（用户要求）

- 成员库默认角色只保留 **项目牧羊人**（领队）与 **角色构建师** 两个系统成员。
- 4 个预置成员（前端开发者 / 后端架构师 / UI 设计师 / 趣味注入师）不再自动入库：`PRESET_MEMBER_ROLES` 收敛为仅 角色构建师；`ROLE_TEMPLATES` 仍保留，手动新增成员按角色标签照常套用人设。
- 批量导入的 186 个角色已从成员库移除；全部手册仍以文件形态保留在本目录，需要时按名重建或走 `eTeam --add-people`。
- 清理前的完整成员库备份：`C:\eTeam\.tmp-roster-backup-2026-09-01T02-54-13-440Z.json`（确认无需回滚后可删除）。
