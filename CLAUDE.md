...
- **Schema v17，16 表**：`schema_meta`（键值元数据）、`team`、`roles`（角色库；`employee_id` 弃用列保留）、`team_members`（班底，行自增主键即工牌 + 副本列）、`task`（大/小任务：合同 MD、成员链（站点=工号数字，legacy 名字串兼容）、链游标 -1=未开始、重试计数、状态说明、blockedFrom、work_dir=当前会话目录（建任务时冻结，绝对）/ task_dir=任务目录（相对 work_dir））、`task_members`（任务成员副本，建任务整班复制；领队主持行 `name='团队领队'`、`main_task_id` 空）、`attempts`（一行一次尝试：kind=initial/stage/retry/reassign、一次性 token、`task_member_id` 归属精确到副本行、进度 JSON、结果、变更文件）、`events`（只追加审计）、`mail_messages`（收件箱按 (team_id, employee_id) 分箱，`message_id` 幂等）、`decisions`（升级决策）、`task_status_changes`、`usage_detail` + `usage_daily_total`（用量，DB 即唯一存储）、`ask_questions`（问答弹窗路由）、`capability_gaps` + `routing_memos`（v16 子代理能力缺口与常设路线）。
...
- **schema.sql 与 db.ts 双份**：[schema.sql](src/host/state/schema.sql) 必须与 `src/host/state/db.ts` 内嵌 `SCHEMA_SQL` 常量**逐字一致**（bundle 内读不到同目录资产故内嵌；schema.sql 是审核对照副本）。改表 = `DB_SCHEMA_VERSION` +1，旧库在 `getDb` 里 ALTER + 回填（形状按 `PRAGMA table_info` 守卫、幂等，重开自愈）。数据不是「能安全推导」的**不硬回填**，只补形状——v17 的 `task.task_dir` 即先例（旧 `work_dir` 是旧语义的相对值，按「不管之前的任务」不回填，旧行由 `docs.isCurrentLayout` 判旧布局、不参与物化）。
- **任务工作文档**（用户 2026-09-15 扁平化）：`<任务 work_dir>/teams/<主任务号>-slug/` —— **一个主任务一个目录**（团队名不进路径，主任务与其全部小任务共用；`work_dir` = 建任务时冻结的当前会话目录，绝对路径，所以目录不随调用者换工作区），目录下 `留言板.md`（create-only，领队+全员共用一块）、每任务一份 `<任务号>-slug.纪要.md`（上半段宿主幂等渲染的合同视图 + 下半段纪要正文），以及只建目录、内容自由的 `计划/`、`文档/`。旧布局（`tasks/tN-slug/`、`sub/`、`contract.md`、`notes.md`、团队 `README.md`）不迁移、不兼容。
